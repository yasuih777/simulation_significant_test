...
class AppBuilder:
    ...
    def body_components(self) -> None:
        ...
        with st.container(border=True):
            ...
            with tparam_body[1]:
                alternative = st.selectbox("対立仮説を棄却する向き", ["two-sided", "greater", "less"])
                alpha = st.number_input(
                    "有意水準", min_value=0.0, max_value=1.0, value=0.05, step=0.01
                )
            with tparam_body[2]:
                iters = st.number_input("シミュレーション回数", min_value=1000, value=10000, step=100)
